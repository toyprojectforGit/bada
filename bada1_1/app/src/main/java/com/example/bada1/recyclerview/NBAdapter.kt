...
class NBAdapter(interfaceRecyclerView: INBAdapter) : RecyclerView.Adapter<DataViewHolder>() {
    private var datalist = ArrayList<NoticeModel>()
    private var Recyclerview:INBAdapter=interfaceRecyclerView
    init {
...
class DataViewHolder( itemView:View,interfaceRecyclerView: INBAdapter) : RecyclerView.ViewHolder(itemView), View.OnClickListener { //여기서 각각의 데이터 넣어주는 것
    private var Recyclerview:INBAdapter=interfaceRecyclerView
    ...
    val layout = itemView.item_layout
    ...
    override fun onClick(p0: View?) {
        when(p0){
            layout->{
                this.Recyclerview.onSearchItemClicked(position,title.text.toString())
            }
            heartbtn ->{
